...
def map(inp,gt,predicted):
    ...
    for i in range(len(gt)):
        #word is changed and is same as predicted
        if inp[i]!=predicted[i] and gt[i]==predicted[i]:
            p+=1
        if inp[i]==gt[i]:
            q+=1
    return p/(p+q)
...
if custom:
    gt_array=['this is a ball', 'how are you', 'i am a boy', 'this has to be done']
    inp_array1=['tis is a bll', 'hw r u', 'i m a boi', 'tis hs to be dn']    
else:
    gt_array=[]
    inp_array=[]
...
uni=bigram_model.bigram_auto()
uni.create_bigram()
#str1=uni.model(query='hw are you')
#print(str1)
#bre
#==============================================================================
